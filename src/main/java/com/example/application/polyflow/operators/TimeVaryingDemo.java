...
public class TimeVaryingDemo implements TimeVarying<List<Tuple>> {
    ...
    public TimeVaryingDemo(StreamToRelationOperator<Tuple,Tuple, TuplesOrResult> op, String name) {
        this.op = op;
        this.name = name;
    }
    ...
    public void materialize(long ts) {
        /*content = op.getContents(ts).stream().map(Content::coalesce).reduce(new TuplesOrResult(), (l1, l2)-> {
            l1.addAll(l2);
            return l1;
        });*/
    }
